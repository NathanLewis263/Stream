...
function validateShortcut(data: CapturedKeyData, platform: string): ValidationResult {
  ...
  if (reservedShortcuts.has(data.displayName)) {
    return { valid: false, error: "Reserved system shortcut" };
  }
      ...
      <div className="relative flex items-start justify-between gap-6">
        <div className="flex items-start gap-4 flex-1 min-w-0">
          <div
            className={`
              ...
              ${isCapturing
                ? "bg-[#4d65ff]/15 text-[#4d65ff] border border-[#4d65ff]/20"
        ...
        {isCapturing ? (
          <div className="flex items-center gap-2 shrink-0">
            <div
              className={`
                px-4 py-2 rounded-xl font-mono text-sm min-w-[140px] text-center border
                ${isValid
                  ? "bg-teal-500/10 border-teal-500/25 text-teal-300"
                  : validationError
                    ? "bg-red-500/10 border-red-500/25 text-red-300"
                    : "bg-[#4d65ff]/10 border-[#4d65ff]/25 text-[#4d65ff]"
                }
              `}
            >
            ...
            <button
              onClick={onCancel}
              className="px-4 py-2 rounded-xl text-sm font-medium bg-zinc-800 text-zinc-300 hover:bg-zinc-700 border border-white/5"
            >
              Cancel
            </button>
          </div>
          ...
          <button
            ...
            className="group shrink-0 px-5 py-2 rounded-xl font-mono text-sm bg-zinc-800/80 text-zinc-200 hover:bg-zinc-700 border border-white/6 hover:border-white/10 min-w-[140px] transition-all duration-200"
          >
      ...
      {isCapturing && (
        <div
          className={`
            ...
            ${validationError ? "text-red-400" : isValid ? "text-teal-400" : "text-zinc-500"}
          `}
        >
          {validationError ? (
            <>
...
export const HotkeySettings = () => {
  ...
  const InfinityIcon = (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 12c0-2.5 2-4.5 4.5-4.5 1.657 0 3.107.895 3.89 2.228L12 10l-.39.272c.783 1.333 2.233 2.228 3.89 2.228C18 12.5 20 10.5 20 8s-2-4.5-4.5-4.5c-1.657 0-3.107.895-3.89 2.228M4 12c0 2.5 2 4.5 4.5 4.5 1.657 0 3.107-.895 3.89-2.228M20 12c0 2.5-2 4.5-4.5 4.5-1.657 0-3.107-.895-3.89-2.228" />
    </svg>
  );
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 leading-relaxed">
              Click to record a new shortcut. Hold the keys, then save.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-zinc-800/60 text-xs text-zinc-400 border border-white/5">
                <span className="w-1.5 h-1.5 rounded-full bg-[#4d65ff]/60" />
                Include modifier
              </span>
            </div>
          </div>
      ...
      <div className="space-y-3">
        ...
        <HotkeyItem
          ...
          description="Toggle continuous recording"
          icon={InfinityIcon}
      ...
      <div className="pt-5 border-t border-white/[0.04]">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-zinc-400 hover:text-zinc-200 bg-zinc-800/50 hover:bg-zinc-800 border border-white/5 transition-all duration-200"
        >
