...
def create_app(engine_ref):
    ...
    async def lifespan(app: FastAPI):
        ...
        def bridge_to_async(topic, data):
            asyncio.run_coroutine_threadsafe(
                manager.broadcast({"type": topic, "data": data}), running_loop
            )
        ...
        engine_ref.on_processing_error = lambda d: bridge_to_async("processing_error", d)

        if hotkey_key_captured_callback_setter:
            hotkey_key_captured_callback_setter(
                lambda data: bridge_to_async("key_captured", data)
            )
    ...
    async def websocket_endpoint(websocket: WebSocket):
        ...
        try:
            ...
            ptt_key = hotkeys.get("push_to_talk", {}).get(platform, {}).get(
                "key", "fn" if platform == "darwin" else "Ctrl+Win"
            )
                    ...
                    if action == "start":
                        ...
                        with engine_ref.lock:
                            if engine_ref.is_recording:
                                was_cmd = engine_ref.is_command_mode
                                audio_data = engine_ref.stop_recording()
                                threading.Thread(
                                    target=lambda: engine_ref.process_audio(
                                        audio_data, command_mode=was_cmd
                                    )
                                ).start()
                        ...
                        if instruction and selected_text:
                            threading.Thread(
                                target=lambda: engine_ref.process_editor_command(
                        ...
                        if hotkey_start_capture_callback:
                            ...
                            await websocket.send_json(
                                {"type": "capture_started", "data": {"success": True}}
                            )
                        ...
                        if hotkey_stop_capture_callback:
                            ...
                            await websocket.send_json(
                                {"type": "capture_stopped", "data": {"success": True}}
                            )
    ...
    def get_last_dictation():
        """Debug / integration: last raw vs refined text from the engine (not used by the overlay UI)."""
        return {
...
def run_status_server(engine_ref):
    """Run FastAPI + WebSocket on STATUS_SERVER_PORT."""
